...
The MSDN code makes use of a `reducibleNodes` array that is built while populating the Octree. It's essentially a 2D jagged array of all the nodes in the tree; the first index is the level, with the second index being an unordered (in principal anyway) list of the nodes at that level. *(Note that it is not actually a 2D jagged array! It's an array of references to the first node in the list, and then the nodes themselves form a linked list by way of their `NextReducible` property. Yuck!)*
